...
Now let's prettify the output. Let's name the columns "width" and "height" and also produce a column for the number of tiles in the inner (which equals the number of tiles at the border).
...
Now we want to add a flag to this data frame if the number `a` is a so called *perfect number*. A perfect number is characterized by being identical to their sum of divisors. To this end we use a substitution term for the sum of divisors to reuse for checking if the number is perfect:
